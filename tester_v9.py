import gym
from c_elegans_env_v9 import CEMazeEnv1, CEMazeEnv2, TrainingStatsCallback, ModelCheckpointCallback # type: ignore
...
for attempt, trial_seed in enumerate(seeds, start=1):
    ...
    model = SAC(
        ...
        verbose=1,
        learning_rate=3e-4,
        buffer_size=150_000, # 400000 for 2.5M, alpha = 0.1 and 500K for 1.5M, alpha = 0.1 (second one better)
    ...
    # Create checkpoint callback to save every 500K steps
    checkpoint_callback = ModelCheckpointCallback(
        save_freq=100_000, # CHANGE
        save_path="RL_models",
    ...
    model.learn(total_timesteps=500_000, callback=combined_callback)

    # Save model
    model_filename = f"RL_models/trial{trial_number}_atmp{trial_seed}_sac_500K" # 2.5 M or 600K

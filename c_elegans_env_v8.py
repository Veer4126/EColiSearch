...
class CEMazeEnv(gym.Env):
    ...
    def __init__(self, size=8, max_steps=500, k_w2r=0.5, k_r2w=0.5, alpha=0.1, gradient_sigma=2.0, noise_std=0.01):
        super().__init__()
        ...
        self.noise_std = noise_std # noise
    ...
    def _get_observation(self, current_conc=None):
        ...
        noisy_prev_conc = self.prev_conc + np.random.normal(0, self.noise_std)
        noisy_current_conc = current_conc + np.random.normal(0, self.noise_std)
        noisy_prev_p_reorient = self.prev_p_reorient + np.random.normal(0, self.noise_std)
        noisy_prev_p_walk = self.prev_p_walk + np.random.normal(0, self.noise_std)

        return np.array([
            noisy_prev_conc,
            noisy_current_conc,
            np.clip(noisy_prev_p_reorient, 0.0, 1.0),
            np.clip(noisy_prev_p_walk, 0.0, 1.0)
        ], dtype=np.float32)
    ...
    def step(self, action=None):
        ...
        noisy_curr_conc = obs[1] # Noisy values for post-training analysis
        noisy_prev_conc = obs[0]
        ...
        return obs, reward, terminated, truncated, {
            ...
            "noisy_concentration": noisy_curr_conc,
            "noisy_delta_conc": noisy_curr_conc - noisy_prev_conc,
...
class TrainingStatsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # Concentration
        noisy_conc  = info.get("noisy_concentration", None)
        if noisy_conc  is not None:
            self.current_episode_concs.append(noisy_conc)

            # Delta C tracking
            if hasattr(raw_env, "prev_noisy_conc"):
                delta_conc = noisy_conc - raw_env.prev_noisy_conc
                raw_env.gradient_rates.append(delta_conc)
                raw_env.prev_noisy_conc = noisy_conc  # update for next step

            # Peak dwell tracking
            if hasattr(raw_env, "peak_times"):
                if noisy_conc >= self.peak_threshold:

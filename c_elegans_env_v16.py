# Sparse reward, with gradient, observation vector WITHOUT memory [prev_conc, curr_conc, kappa, motion_state]
...
class CEMazeEnv(gym.Env):
    ...
    def __init__(self, size=10, max_steps=500, alpha=0.5, gradient_sigma=1.0):
        super().__init__()
        ...
        self.motion_state = WormState.FORWARD
        self.time_in_state = 0

        self.kappa = 0.0  # angular curvature
        ...
        # Observation Space
        # [prev_conc, curr_conc, kappa, motion_state]
        self.observation_space = spaces.Box(
            low=np.array([0.0, 0.0, -np.inf, 0.0]),
            high=np.array([np.inf, np.inf, np.inf, 3.0]),
            dtype=np.float32
        )
    ...
    def get_concentration(self, pos): # get the concentration at each position to inform the state of the agent and determine the next step
        ...
        return sum(
            self.peak_amplitude * np.exp(-np.linalg.norm(pos - tgt)**2 / (2 * self.gradient_sigma**2))
    ...
    def reset(self, seed=None, options=None):
        ...
        return self._get_observation(current_conc=self.prev_conc), {} # prev_conc is reset to 0 and curr_conc = prev_conc in this line. So state vector = [0.0, 0.0, 0.5, 0.5]
    ...
    def step(self, action=None):
        ...
        if action is not None:
            ...
            # FORWARD
            probs = normalize(a[0:3])
            probs /= probs.sum() + 1e-8
            P[WormState.FORWARD] = {
                WormState.FORWARD: probs[0],
                WormState.PAUSE: probs[1],
                WormState.REVERSAL: probs[2]
            }

            # PAUSE
            probs = normalize(a[3:6])
            probs /= probs.sum() + 1e-8
            P[WormState.PAUSE] = {
                WormState.FORWARD: probs[0],
                WormState.PAUSE: probs[1],
            ...
            # REVERSAL
            probs = normalize(a[6:10])
            probs /= probs.sum() + 1e-8
            P[WormState.REVERSAL] = {
                WormState.FORWARD: probs[0],
                WormState.PAUSE: probs[1],                
                WormState.REVERSAL: probs[2],
                WormState.OMEGA: probs[3]
            }

            # OMEGA (forced exit)
            P[WormState.OMEGA] = {
                WormState.FORWARD: 1.0
            }
        ...
        # --- FSM transition ---
        states, probs = zip(*P[self.motion_state].items())
        ...
        if self.motion_state in [WormState.FORWARD, WormState.REVERSAL]:

            rho = 0.75 if self.motion_state == WormState.FORWARD else 0.5
            sigma = 0.05 if self.motion_state == WormState.FORWARD else 0.15

            self.kappa = rho * self.kappa + np.random.normal(0, sigma)

            direction = 1.0 if self.motion_state == WormState.FORWARD else -1.0

            # orientation angle factors in kappa (directional bias)
            self.theta += direction * self.kappa
        ...
        elif self.motion_state == WormState.OMEGA:

            if self.time_in_state == 0:
                self.theta += np.random.choice([-1, 1]) * np.random.uniform(np.pi, 1.5*np.pi)

        elif self.motion_state == WormState.PAUSE:
            pass            
        ...
        # Log peak dwelling
        self.peak_times.append(int(curr_conc > 0.95 * self.peak_amplitude))
        ...
        obs = self._get_observation(curr_conc) # This is where the 2 probabilities and 2 concs are added to the state vector.
        terminated = False
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            ...
            metrics = {
                ...
                "episode_avg_conc_delta": self.stats_callback.episode_avg_conc_delta,
            }

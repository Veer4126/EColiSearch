...
for attempt, trial_seed in enumerate(seeds, start=1):
    ...
    model = SAC(
        ...
        verbose=1,
        learning_rate=3e-4,
        buffer_size=800_000, # 150K for 500K iterations
    ...
    model.learn(total_timesteps=2_500_000, callback=combined_callback)

    # Save model
    model_filename = f"RL_models/trial{trial_number}_atmp{trial_seed}_sac_2_5M" # 2.5 M
...
#     model.learn(total_timesteps=10_000, callback=combined_callback)

# cProfile.run("train()", "profiler_stats")

# Single State Exponential Agent, Sparse reward, no gradient, state vector WITHOUT memory [prev_lambda]
...
class CEMazeEnv(gym.Env):
    ...
    def __init__(self, size=20, max_steps=500, alpha=0.5, hit_radius=0.25):
        ...
        self.prev_lambda = 2.0 # measured from original naive agent
        self.curr_lambda = 2.0 # measured from original naive agent
    ...
    def step(self, action=None):
        ...
        if action is not None:
            self.curr_lambda = float(action)

        self.curr_lambda = np.clip(self.curr_lambda, 0.1, 20.0)# just in case             
                
        # Sample ONE run length
        L = int(np.ceil(np.random.exponential(self.curr_lambda)))

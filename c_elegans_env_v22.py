# Single State Agent, Multiple sparse rewards, no gradient, state vector WITHOUT memory [prev_p_w2w]
...
class CEMazeEnv(gym.Env):
    ...
    def __init__(self, size=200, max_steps=2000, step_size=1.0, hit_radius=0.25):
        ...
        # Observation: [prev_p_w2w]
        self.observation_space = spaces.Box(
            low=np.array([0.0]),       # min allowed value
            high=np.array([1.0]),      # max allowed value
            dtype=np.float32
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        self.targets_array = np.array(self.targets)  # shape (n_targets, 2)
    ...
    def step(self, action=None):
        ...
        lifted_targets = self.targets_array + self.size * np.round(
            (prev_true_pos - self.targets_array) / self.size
        )
        ...
        final_lifted_targets = self.targets_array + self.size * np.round(
            (self.true_agent_pos - self.targets_array) / self.size
        )
        ...
        obs = self._get_observation() # This is where the probability is added to the state
        terminated = False

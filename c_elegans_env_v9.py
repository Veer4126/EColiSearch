...
class CEMazeEnv1(gym.Env):
    ...
    def __init__(self, size=10, max_steps=2500, p_w2r=0.5, p_r2w=0.5, alpha=0.1, gradient_sigma=2.0):
        ...
        self.prev_p_w2r = 0.5  # Initial value for obs
        self.prev_p_r2w = 0.5
...
class ModelCheckpointCallback(BaseCallback):
    def __init__(self, save_freq, save_path, trial_number, trial_seed, verbose=0):
        super().__init__(verbose)
    ...
    def _on_step(self) -> bool:
        if self.num_timesteps % self.save_freq == 0:
            filename = f"trial{self.trial_number}_atmp{self.trial_seed}_step{self.num_timesteps // 1000}K"
            path = os.path.join(self.save_path, filename)
            self.model.save(path)
            if self.verbose > 0:
                print(f"[Checkpoint] Saved model at step {self.num_timesteps} to {path}")
        return True
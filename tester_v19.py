import gym
from c_elegans_env_v19 import CEMazeEnv, TrainingStatsCallback, ModelAndMetricsCheckpointCallback # type: ignore
...
for attempt, trial_seed in enumerate(seeds, start=1):
    ...
    def plot_metrics():
        ...
        # Mean Rate Constants
        plt.subplot(2, 2, 1)
        plt.plot(mean_actions[:, 0], label='Mean Alpha', linewidth=0.8)
        ...
        plt.subplot(2, 2, 2)
        plt.plot(stddevs[:, 0], label='Stddev Alpha', linewidth=0.8)
        plt.title("Stddev of Alpha Over Time")
        plt.xlabel("Step")
        plt.ylabel("Stddev")
        plt.grid(True)
        plt.legend()

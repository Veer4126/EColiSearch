import gym
from c_elegans_env_v12 import CEMazeEnv1, CEMazeEnv2, TrainingStatsCallback, ModelAndMetricsCheckpointCallback # type: ignore
...
# Curriculum schedule: (maze size, training steps)
curriculum = [
    (4, 625_000),
    (6, 625_000),
    (8, 625_000),
...
for attempt, trial_seed in enumerate(seeds, start=1):
    print(f"\n[INFO] Starting Trial {trial_number} Attempt {trial_seed}...\n")
    np.random.seed(trial_seed)
    th.manual_seed(trial_seed)

    # Start with smallest environment
    env = CEMazeEnv2(size=curriculum[0][0])
    env.seed(trial_seed)
    ...
    for grid_size, steps in curriculum:
        print(f"\n[Curriculum] Training with maze size {grid_size} for {steps:,} timesteps...\n")

        # Create new environment for this stage
        new_env = CEMazeEnv2(size=grid_size)
        new_env.seed(trial_seed)
        model.set_env(new_env)

        # Train on this grid size
        model.learn(total_timesteps=steps, reset_num_timesteps=False, callback=combined_callback)

        total_trained_steps += steps
        print(f"[Curriculum] Finished training {steps:,} steps (Total: {total_trained_steps:,})\n")

    # After curriculum completes, save model
    model.save(f"RL_models/trial{trial_number}_atmp{trial_seed}_sac_curriculum_final")
    print("[INFO] Curriculum training complete!")

# Sparse reward, no gradient, state vector WITH memory [prev_p_w2r, prev_p_r2w, time_since_hit]
...
class CEMazeEnv2(gym.Env):
    ...
    def __init__(self, size=10, max_steps=500, p_w2r=0.5, p_r2w=0.5, alpha=0.5):
        ...
        # Observation: [prev_p_w2r, prev_p_r2w, time_since_hit]
        self.observation_space = spaces.Box(
            low=np.array([0.0, 0.0, 0.0]),
            high=np.array([1.0, 1.0, float(self.max_steps)]),
            dtype=np.float32
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        self.time_since_hit = 0  # reset memory
    ...
    def step(self, action=None):
        ...
        reward = 0.0
        hit = False
        if np.linalg.norm(self.agent_pos - self.targets[0]) < 0.24:
            self.target_hits += 1
            hit = True
            reward = 1.0  # reward only if target reached
            self.time_since_hit = 0  # reset memory on hit
        else:
            self.time_since_hit += 1 # increment otherwise
    ...
    def render(self, mode='human'):
        ...
        # Target
        ax.plot(*self.targets[0], 'ro', markersize=6)

        # Agent arrow
        ax.arrow(
            self.agent_pos[0],
            self.agent_pos[1],
            0.3 * np.cos(self.theta),
            0.3 * np.sin(self.theta),
            width=0.05,
            color='blue'
        )

        ax.set_title(
            f"Step {self.steps} | {self.motion_state} | "
            f"p_w2r={self.p_w2r:.2f}, p_r2w={self.p_r2w:.2f} | "
            f"Hits={self.target_hits}"
        )
...
# class CEMazeEnvNaive(gym.Env):
#     metadata = {'render.modes': ['human']}

#     def __init__(self, size=10, max_steps=500, p_w2r=0.5, p_r2w=0.5, alpha=0.5, hit_reward=1.0, hit_radius=0.5):
#         super().__init__()
#         self.size = size
#         self.max_steps = max_steps
#         self.alpha = alpha
...
#     def step(self, action=None):  # Naive agent ignores the action!

#         prev_true_pos = self.true_agent_pos.copy()

#         # Do not update rate constants
#         self.p_w2r = self.prev_p_w2r
#         self.p_r2w = self.prev_p_r2w
...
#         # Apply periodic boundary conditions (wrap-around effect)
#         self.true_agent_pos += np.array([dx, dy])  # unwrapped
#         self.agent_pos = self.true_agent_pos % self.size  # wrapped around both x and y coordinates for simulation
...
#         reward = 0.0
#         hit = False
#         if np.linalg.norm(self.agent_pos - self.targets[0]) < 0.24: # uses unwrapped position
#             hit = True
#             reward = 1.0
#             self.time_since_hit = 0  # reset memory on hit
#         else:
...
#         # delta = np.abs(self.agent_pos - self.targets[0])
...
#         # if dist < self.hit_radius and not self.has_hit:
#         #     hit = True

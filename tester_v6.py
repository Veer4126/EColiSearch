import gym
from c_elegans_env_v6 import CEMazeEnv1, CEMazeEnv2, TrainingStatsCallback # type: ignore
...
for attempt, trial_seed in enumerate(seeds, start=1):
    ...
    model = SAC(
        ...
        verbose=1,
        learning_rate=3e-4,
        buffer_size=500000, # 400000 for 2.5M, alpha = 0.1 and 500K for 1.5M, alpha = 0.1 (second one better)
    ...
    # # Save model
    # model_filename = f"RL_models/trial{trial_number}_atmp{trial_seed}_sac_1_5M" # 2.5 M or 600K
    ...
    def plot_metrics():
        ...
        bucket_size = 100
        max_steps =2500  # Or the maximum number of steps per episode
        ...
        plt.plot([std[0] for std in callback.stddevs], label="Stddev k_w2r", linewidth=0.8)
        plt.plot([std[1] for std in callback.stddevs], label="Stddev k_r2w", linewidth=0.8)
        ...
        if len(callback.stddevs) >= window:
            ...
            plt.plot(range(window - 1, len(callback.stddevs)), rolling_std_k_w2r, label=f"{window}-Episode Rolling Avg (k_w2r)", color='red', linestyle='--')
            plt.plot(range(window - 1, len(callback.stddevs)), rolling_std_k_r2w, label=f"{window}-Episode Rolling Avg (k_r2w)", color='blue', linestyle='--')
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, "stddev_rate_constants_over_time.png"))
        plt.close()
        ...
        # STEP LENGTHS
        plt.figure(figsize=(6, 4))
        min_val, max_val = min(callback.step_lengths), max(callback.step_lengths)
        bins = np.linspace(min_val, max_val, 50)
        plt.hist(callback.step_lengths, bins=bins, edgecolor='black', alpha=0.7)
        plt.xlabel("Step Length")
        plt.ylabel("Frequency")
        plt.title("Training Step Length Distribution")
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, "step_length_distribution.png"))
        plt.close()

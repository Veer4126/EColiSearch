# Single State Agent, Sparse reward, no gradient, state vector WITH memory [prev_p_w2w, time_since_hit]
...
def segment_circle_hit(p0, p1, c, r):
    """
...
class CEMazeEnv(gym.Env):
    ...
    def __init__(self, size=10, max_steps=500, p_w2w=0.5, alpha=0.5, hit_radius=0.5):
        super().__init__()
        ...
        self.prev_p_w2w = 0.5  # Initial value for obs
        ...
        # Observation: [prev_p_w2w, time_since_hit]
        self.observation_space = spaces.Box(
            low=np.array([0.0, 0.0]),
            high=np.array([1.0, float(self.max_steps)]),
            dtype=np.float32
        )
    ...
    def step(self, action=None):
        ...
        reward = 0.0
        hit = segment_circle_hit( # detect for hit at every timestep of every episode.
            prev_true_pos % self.size, # previous wrapped position
            self.agent_pos, # current wrapped position
            self.targets[0], # target's wrapped position
            self.hit_radius
        )
        if hit:

            if not self.has_hit: # First hit ever in this episode
                reward = 1.0
                self.has_hit = True
                self.time_since_hit = 0

                if self.first_hit_step is None:

...
# OPTIMIZED VERSION OF V20: Vectorized distance checks, vectorized dwell detection, vectorized target relocation, and other optimizations for speed and efficiency
...
class CEMazeEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset targets for the new episode
        self.targets = np.random.uniform(0, self.size, size=(self.n_targets, 2))  # ndarray, not list
    ...
    def step(self, action=None):
        ...
        lifted_targets = self.targets + self.size * np.round(
            (prev_true_pos - self.targets) / self.size
        )
        ...
        final_lifted_targets = self.targets + self.size * np.round(
            (self.true_agent_pos - self.targets) / self.size
        )

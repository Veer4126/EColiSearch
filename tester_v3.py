import gym
from c_elegans_env_v3 import CEMazeEnv, TrainingStatsCallback # type: ignore
...
model = SAC(
    ...
    learning_starts=1000,       # Start learning after some random steps
    batch_size=256,
    tau=0.01,                  # Target smoothing
    gamma=0.99,
    train_freq=1,
    gradient_steps=2,
    ent_coef="auto"            # Let SAC tune entropy itself!
)

# print(model.policy.action_dist.__class__)  #  DiagGaussianDistribution
print(model.policy)
...
model.save("RL_models/trial9_sac_200k_stdhyps_no_bonus_penalty")
...
# Mean Action Plot
plt.subplot(2, 2, 1)
plt.plot(mean_actions[:, 0], label='Mean p_reorient', linewidth=0.8)
plt.plot(mean_actions[:, 1], label='Mean dtheta', linewidth=0.8)
plt.plot(mean_actions[:, 2], label='Mean alpha', linewidth=0.8)
...
# Stddev Plot
plt.subplot(2, 2, 2)
plt.plot(stddevs[:, 0], label='Stddev p_reorient', linewidth=0.8)
plt.plot(stddevs[:, 1], label='Stddev dtheta', linewidth=0.8)
plt.plot(stddevs[:, 2], label='Stddev alpha', linewidth=0.8)
...
motion_counts = Counter(callback.motion_history)
...
#-----------------------------------------------------------------------------------------------------------
# PLOT α (STEP SIZE) AND θ (HEADING ANGLE) OVER TIME
plt.figure(figsize=(12, 5))

# Alpha plot
plt.subplot(1, 2, 1)
plt.plot(callback.alphas, label='Alpha (Step Size)', color='blue', alpha=0.7)
plt.title("Alpha (Step Size) Over Time")
plt.xlabel("Step")
plt.ylabel("Alpha")
plt.grid(True)
...
# Theta plot
plt.subplot(1, 2, 2)
plt.plot(callback.thetas, label='Theta (Heading)', color='purple', alpha=0.7)
plt.title("Theta (Heading Angle) Over Time")
plt.xlabel("Step")
plt.ylabel("Theta (radians)")
plt.grid(True)

plt.tight_layout()
plt.savefig(os.path.join(save_dir, "step_size_and_heading_angle.png"))
plt.close()


#-----------------------------------------------------------------------------------------------------------
# PLOT CHANGE IN THETA OVER TIME (WHEN TURNING)
plt.figure(figsize=(10, 4))
plt.plot(callback.dthetas, label='dTheta (Change in Heading)', color='darkgreen', alpha=0.6)
plt.title("dTheta (Turn Magnitude) Over Time")
plt.xlabel("Step")
plt.ylabel("dTheta (radians)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "change_in_theta.png"))
plt.close()

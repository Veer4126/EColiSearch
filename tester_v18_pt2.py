import gym
from c_elegans_env_v18 import CEMazeEnv, TrainingStatsCallback, ModelAndMetricsCheckpointCallback # type: ignore
...
# DEFAULT MODEL HYPERPARAMETERS FOR PPO
...
#  'ent_coef': 0.0,
#  'env': <stable_baselines3.common.vec_env.dummy_vec_env.DummyVecEnv object at 0x7856e5f4c980>,
#  'ep_info_buffer': None,
#  'ep_success_buffer': None,
#  'gae_lambda': 0.95,
#  'gamma': 0.99,
#  'learning_rate': 0.0003,
#  'lr_schedule': FloatSchedule(ConstantSchedule(val=0.0003)),
#  'max_grad_norm': 0.5,
#  'n_envs': 1,
#  'n_epochs': 10,
#  'n_steps': 2048,
...
#  'policy_kwargs': {},
#  'rollout_buffer': <stable_baselines3.common.buffers.RolloutBuffer object at 0x7856e5f4d010>,
...
for attempt, trial_seed in enumerate(seeds, start=1):
    ...
    model = PPO(
        ...
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,       # IMPORTANT for exploration
    )
    ...
    # Combine with the existing training stats callback
    combined_callback = CallbackList([callback, checkpoint_callback])

    model.learn(total_timesteps=100_000, callback=combined_callback)

    # Save model
    model_filename = f"RL_models/trial{trial_number}_atmp{trial_seed}_ppo_20M" # 2.5 M or 600K
    ...
    def plot_metrics():
        ...
        # Mean Rate Constants
        plt.subplot(2, 2, 1)
        plt.plot(mean_actions[:, 0], label='Mean Lambda', linewidth=0.8)
        plt.title("Mean Lambda Over Time")
        plt.xlabel("Step")
        ...
        plt.subplot(2, 2, 2)
        plt.plot(stddevs[:, 0], label='Stddev Lambda', linewidth=0.8)
        plt.title("Stddev of Lambda Over Time")
        plt.xlabel("Step")
        plt.ylabel("Stddev")
        plt.grid(True)
        plt.legend()
        ...
        plt.plot([std[0] for std in callback.stddevs], label="Stddev Lambda", linewidth=0.8)
        
        window = 100
        if len(callback.stddevs) >= window:
            rolling_std_lambda = np.convolve([std[0] for std in callback.stddevs], np.ones(window)/window, mode='valid')
            plt.plot(range(window - 1, len(callback.stddevs)), rolling_std_lambda, label=f"{window}-Episode Rolling Avg (Lambda)", color='red', linestyle='--')

        plt.title("Standard Deviation of Lambda Over Time")
        plt.xlabel("Episode")
        plt.ylabel("Standard Deviation")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, "stddev_lambda_over_time.png"))
        plt.close()
